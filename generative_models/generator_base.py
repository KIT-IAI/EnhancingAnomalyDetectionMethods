    ...
    def _get_conds(self, kwargs, rdx, x):
        ...
        for key, value in kwargs.items():
            ...
            if key == "cal_input":
                ...
                cal_val = value[rdx]
            elif key == "stats_input":
                #value = value.repeat(96, axis=1).reshape((-1, 96, 1))
                stats_train = np.delete(value, rdx, axis=0)
                stats_val = value[rdx]
        return np.concatenate([cal_train, stats_train], axis=1), np.concatenate([cal_val, stats_val], axis=1),  x_train, x_val
    ...
    def _get_conditions(self, kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if key == "cal_input":
                ...
                stats = value
        return np.concatenate([cals, stats], axis=1)
